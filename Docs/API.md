...
Re-rig with user-placed landmarks from the editor. Required body:
...
Coordinates are world-space floats from the Three.js editor. Returns `202 Accepted` immediately; poll `/status/`.

Missing keys → `400` with the list of missing names. The pipeline preserves the previous output on failure for the same reason as `/rerig/`.

For the landmark anatomy, see [RIGGING_PIPELINE](RIGGING_PIPELINE.md#landmarks).
